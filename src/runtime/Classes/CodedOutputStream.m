...
static const int32_t DEFAULT_BUFFER_SIZE = 4 * 1024;
...
+ (PBCodedOutputStream*)streamWithData:(NSMutableData*)data {
  return [[[PBCodedOutputStream alloc] initWithData:data] autorelease];
}


- (void)flush {
  if (data_) {
    // Don't do anything if we're writing to a NSData object.
    return;
  }
  if (output == nil) {
...
- (void)writeRawData:(const NSData*)value offset:(int32_t)offset length:(int32_t)length {
  if (data_) {
    NSData* subrange = [value subdataWithRange:NSMakeRange(offset, length)];
    [data_ appendData:subrange];
  } else {
    while (length > 0) {
